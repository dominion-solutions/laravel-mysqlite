...
{
    //phpcs:disable
    public static function mysql_convert_tz(?string $date, ?string $fromTimezone, ?string $toTimezone): ?string
    {
        //phpcs:enable
        if ($date && $fromTimezone && $toTimezone) {
            ...
            $converted = Carbon::parse($date, $fromTimezone);
    ...
    //phpcs:disable
    public static function mysql_date_format(?string $date, ?string $format): ?string
    {
        //phpcs:enable
        $dictionary = [
            '%a' => 'D',
    ...
    // phpcs:disable
    public static function mysql_hour(?string $time): ?string
    {
        //phpcs:enable
        if ($time) {
            $asTime = Carbon::parse($time);
    ...
    // phpcs:disable
    public static function mysql_minute(?string $time): ?string
    {
        // phpcs:enable
        if ($time) {
            $asTime = Carbon::parse($time);

            return $asTime->format('i');
        }
    ...
    // phpcs:disable
    public static function mysql_timediff(?string $timeExpression1, ?string $timeExpression2): ?string
    {
        // phpcs:enable
        if ($timeExpression1 && $timeExpression2) {
            $dateTime1 = Carbon::parse($timeExpression1);
            $dateTime2 = Carbon::parse($timeExpression2);
            $dateTimeInterval = $dateTime2->diff($dateTime1);
            $days = $dateTimeInterval->d;
            $hours = ($days * 24) + $dateTimeInterval->h;
            $hourFormatter = new \NumberFormatter(\Locale::getDefault(), \NumberFormatter::PATTERN_DECIMAL, '00');
    ...
    // phpcs:disable
    public static function mysql_timestampdiff(string $timeUnit, int|string|null $startTimeStamp, int|string|null $endTimeStamp): int|string|null
    {
        //phpcs:enable
    ...
    public static function mysql_time_to_sec(?string $timeExpression): int|null|string
    {
        ...
        if ($timeExpression !== null) {
            ...
            $time = Carbon::parse($timeExpression);
    ...
    //phpcs:disable
    public static function mysql_utc_timestamp(): int
    {
        //phpcs:enable
    ...
    // phpcs:disable
    public static function mysql_weekday(?string $date): ?int
    {
        //phpcs:enable
        if ($date) {
    ...
    //phpcs:disable
    public static function mysql_week(?string $date, int $mode): int|string|null
    {
        //phpcs:enable
        if ($date) {
    ...
    //phpcs:disable
    public static function mysql_yearweek(?string $date, int $mode): ?string
    {
        //phpcs:enable
        if ($date) {
            $date = CarbonImmutable::parse($date);
            ...
            switch ($mode) {
                case 0: //uses same algorithm as mode 2 of WEEK function
                    ...
                    if ($sow->year < $date->year) {
                        ...
                        return $date->isoWeekYear(null, Carbon::SUNDAY).str_pad((string) $date->isoWeek(null, Carbon::SUNDAY), 2, '0', STR_PAD_LEFT);
                    } else {
                        if ($firstSunday->weekOfYear === 1) {
                            return $date->isoWeekYear(null, Carbon::SUNDAY).str_pad((string) ($date->weekOfYear - 1), 2, '0', STR_PAD_LEFT);
                        } else {
                            return $date->isoWeekYear(null, Carbon::SUNDAY).str_pad((string) $date->weekOfYear, 2, '0', STR_PAD_LEFT);
                        }
                    }
                ...
                case 5: //uses same algorithm as mode 7 of WEEK function
                    ...
                    if ($sow->year < $date->year) { //if the year of the monday at the start of the given week (sow) is the year before the year of the actual date given
                        ...
                        return $date->isoWeekYear(null, Carbon::MONDAY).str_pad((string) $date->weekOfYear, 2, '0', STR_PAD_LEFT);
                    } else {
                        if ($firstMonday->weekOfYear === 1) { //if the week with the first monday on is the first week, then it matches what we want
                            return $date->isoWeekYear(null, Carbon::MONDAY).str_pad((string) $date->weekOfYear, 2, '0', STR_PAD_LEFT);
                        } else { //if the week with the first monday is the second week then we need to subtract a week because it counted the first week as the week before because it have 4 days in it
                            return $date->isoWeekYear(null, Carbon::MONDAY).str_pad((string) ($date->weekOfYear - 1), 2, '0', STR_PAD_LEFT);
                        }
                    }
